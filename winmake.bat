@echo off
rem
rem 4sports Makefile for Windows
rem assembles source code, optionally builds a disk image
rem
rem a qkumba monstrosity from 2021-02-16
rem

setlocal enabledelayedexpansion
set DISK=4sports.hdv
set VOLUME=INSTANT.REPLAY
...
rem https://github.com/sicklittlemonkey/cadius
rem version 1.4.0 or later
set CADIUS=cadius
...
2>build\relbase.log %ACME% -DBUILDNUMBER=%_build% src\4sports.a
for /f "tokens=*" %%q in (build\relbase.log) do set _make=%%q
%ACME% -DBUILDNUMBER=%_build% -DRELBASE=$!_make:~-5,4! -r build\4sports.lst src\4sports.a
goto :EOF
